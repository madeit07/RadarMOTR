...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model: int = 256, n_head: int = 8,
                 n_encoders: int = 6, n_decoders: int = 6,
                 d_ffn: int = 1024, dropout: float = 0.1,
                 activation: str = 'relu', return_intermediate_dec: bool = False,
                 n_feature_levels: int = 4, dec_n_points: int = 4, enc_n_points: int = 4,
                 two_stage: bool = False, two_stage_num_proposals: int = 300,
                 decoder_self_cross: bool = True, sigmoid_attn: bool = False,
                 extra_track_attn: bool = False, memory_bank: bool = False):
        super().__init__()
        ...
        encoder_layer = DeformableTransformerEncoderLayer(d_model, d_ffn,
                                                          dropout, activation,
                                                          n_feature_levels, n_head, enc_n_points,
                                                          sigmoid_attn=sigmoid_attn)
        self.encoder = DeformableTransformerEncoder(encoder_layer, n_encoders)

        decoder_layer = DeformableTransformerDecoderLayer(d_model, d_ffn,
                                                          dropout, activation,
                                                          n_feature_levels, n_head, dec_n_points, decoder_self_cross,
                                                          sigmoid_attn=sigmoid_attn, extra_track_attn=extra_track_attn,
                                                          memory_bank=memory_bank)
        self.decoder = DeformableTransformerDecoder(decoder_layer, n_decoders, return_intermediate_dec)
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        if self.extra_track_attn:
            _log.info('Adding Extra Self Attention in Every Decoder.')
            self.update_attn = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
            self.dropout5 = nn.Dropout(dropout)
            self.norm4 = nn.LayerNorm(d_model)

        if self_cross:
            _log.info('Using Self-Cross Attention.')
        else:
            _log.info('Using Cross-Self Attention.')
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        ...
        n_head=args.nheads,
        n_encoders=args.enc_layers,
        n_decoders=args.dec_layers,
