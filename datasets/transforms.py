...
def pad(image, target, padding):
    ...
    target = target.copy()
    # should we do something wrt the original size?
    target["size"] = torch.as_tensor(padded_image[::-1])
    if "masks" in target:
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        scale = 1 / torch.as_tensor([w, h, w, h], dtype=torch.float32)

        if 'boxes' in target:
            target['boxes'] *= scale
        if 'proposals' in target:
            target['proposals'][..., :4] *= scale
...
class MotTranslateBoxes(object):
    """Translates all bounding boxes from xywh to cxcywh format.
    """
    def __call__(self, images, targets=None):
        ...
        for target in targets:
            target = target.copy()
            if 'boxes' in target:
                target['boxes'] = box_convert(target['boxes'], 'xywh', 'cxcywh')
            if 'proposals' in target:
                target['proposals'][..., :4] = box_convert(target['proposals'][..., :4], 'xywh', 'cxcywh')

            ret_targets.append(target)

        return images, ret_targets
