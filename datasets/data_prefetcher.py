...
def to_device(samples, targets, device):
    samples = samples.to(device, non_blocking=True)
    targets = [{k: v.to(device, non_blocking=True) for k, v in t.items()} for t in targets]
    return samples, targets


def tensor_to_device(tensor: torch.Tensor, device: torch.device, non_blocking: bool = False):
...
def data_dict_to_device(data_dict: dict, device: torch.device, non_blocking: bool = False):
    return data_apply(data_dict, is_tensor_or_instances, partial(tensor_to_device, device=device, non_blocking=non_blocking))
...
class data_prefetcher():
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            self.next_samples, self.next_targets = to_device(self.next_samples, self.next_targets, self.device)
    ...
    def next(self):
        if self.prefetch:
            ...
            try:
                ...
                samples, targets = to_device(samples, targets, self.device)
            except StopIteration:
